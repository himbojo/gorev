...
// App encapsulates the core application state and wiring.
type App struct {
	Config  *Config
	DB      *database.DB
...
// NewApp initializes the application components based on the provided configuration.
func NewApp(cfg *Config) (*App, error) {
	db := database.New(cfg.RedisAddr, cfg.RedisPassword)
	srv := server.New(db, cfg.DataDir, cfg.OCSPEndpoints)
	...
	w, err := watcher.New(cfg.DataDir, app.ReloadPKI)
	if err != nil {
		return nil, err
	}
	app.Watcher = w
...
// ReloadPKI is the callback triggered by the file watcher or at startup.
...
func (a *App) ReloadPKI() {
	...
	// Load Responders
	respDir := filepath.Join(a.Config.DataDir, "responders")
	...
	// cannot be served while revocation sets are being updated.
	if err := a.DB.InvalidateCache(context.Background()); err != nil {
	...
	if crlFiles, err := os.ReadDir(crlDir); err == nil {
		for _, file := range crlFiles {
			...
			// Find the matching CA and verify the CRL signature
...
// SetupRoutes registers the HTTP handlers for OCSP responders and static file servers
// (CRLs, CAs, chains). Returns a new http.ServeMux containing all routes.
func (a *App) SetupRoutes() *http.ServeMux {
	mux := http.NewServeMux()

	// Register OCSP endpoints
	ocspRegistered := make(map[string]bool)
	for _, ep := range a.Config.OCSPEndpoints {
		if !ocspRegistered[ep] {
			mux.HandleFunc(ep, a.Server.HandleOCSP)
		...
		if !ocspRegistered[route] {
			mux.HandleFunc(route, a.Server.HandleOCSP)
	...
	for _, ep := range a.Config.CRLEndpoints {
		route := ep
		if !strings.HasSuffix(route, "/") {
			route += "/"
		}
	...
	for _, ep := range a.Config.CAEndpoints {
		route := ep
		if !strings.HasSuffix(route, "/") {
			route += "/"
		}
	...
	for _, ep := range a.Config.ChainEndpoints {
		route := ep
		if !strings.HasSuffix(route, "/") {
			route += "/"
		}
...
func (h *multiDirHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, dir := range h.dirs {
		...
		// Resolve symlinks and verify the real path is within the allowed directory.
